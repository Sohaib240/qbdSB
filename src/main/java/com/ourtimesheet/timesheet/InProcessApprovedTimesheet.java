 ...
 */
public class InProcessApprovedTimesheet extends Timesheet {

    @PersistenceConstructor
    public InProcessApprovedTimesheet(UUID id, OurDateTime startDate, OurDateTime endDate, Employee employee, List<Signature> signatureList, Note note, int revisionNumber, Map<String, Boolean> exportedConfigurations) {
        super(id, startDate, endDate, employee, signatureList, note, revisionNumber, exportedConfigurations);
    }

    @Override
    public TimesheetStatus getStatus() {
        return TimesheetStatus.PROCESSING_TIMESHEET;
    }

    @Override
    public Timesheet submit(Signature signature) {
        throw new IllegalStateException("Timesheet In Process cannot be submitted");
    }

    @Override
    public Timesheet approve(Signature signature) {
        throw new IllegalStateException("Timesheet In Process cannot be approved");
    }

    @Override
    public Timesheet reject(String rejectionReason) {
        throw new IllegalStateException("Timesheet In Process cannot be rejected");
    }

    @Override
    public Timesheet unSubmit() {
        throw new IllegalStateException("Timesheet In Process cannot be un Submitted");
    }
    ...
    public Timesheet unApprove() {
        throw new IllegalStateException("Timesheet In Process cannot be un Approved");
    }
    ...
    public Timesheet process(OurDateTime timestamp) {
        throw new IllegalStateException("Timesheet In Process cannot be processed");
    }
    ...
    public Timesheet processSuccess(OurDateTime timeStamp, ExportConfigurationType exportConfigurationType, boolean processTimesheet) {
        ...
        if (processTimesheet)
            return new ProcessedTimesheet(super.getId(), super.getStartDate(), super.getEndDate(), super.getEmployee(), super.getSignatureList(), super.getNote(), timeStamp, super.getRevisionNumber(), exportedConfigurations);
        else
    ...
    public Timesheet processFailed() {
        ...
        if (super.containsAnyExportConfiguration())
            ...
            return new ApprovedTimesheet(super.getId(), super.getStartDate(), super.getEndDate(), super.getEmployee(), super.getSignatureList(), super.getNote(), super.getRevisionNumber(), exportedConfigurations);
    }

    @Override
    public Timesheet export(ExportConfigurationType exportConfigurationType) {
        throw new IllegalStateException("In process timesheet cannot be exported");
    }

    @Override
    public Timesheet revise(OurDateTime processedTime) {
        throw new IllegalStateException("In process Approved timesheet cannot be corrected");
    }
